...
def parse_params():
    ...
    parser.add_argument('-epoc_len','--epoch_length', type=int, default=1000, help='# of batches to sample from data slice', required=False)
    ...
    parser.add_argument('-perc','--perc', type=float, default=95., help='percentile threshold for positive label when binarizing y (0-100)', required=False)
    ...
    if args['domain'] == 'shapes':
        ...
        from examples.domains.crism import CRISM as Domain
        from examples.domains.crism import Discriminator, Disentangler
        if args['binarize_y']:
            from examples.domains.crism import AttExtractorBin as AttExtractor
        else:
            from examples.domains.crism import AttExtractor
