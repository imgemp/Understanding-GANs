...
class SimGD(Map):
    ...
    def map(self, real_data, fake_data, data_att, atts, it=None):
        ...
        if self.m.logger is not None and it is not None:
            self.m.logger.histo_summary('data_real', real_data.cpu().data.numpy(), it)
            self.m.logger.histo_summary('data_fake', fake_data.cpu().data.numpy(), it)
            ...
            self.m.logger.scalar_summary('d_probs_real_mean', torch.sigmoid(real_outputs[-1]).mean().cpu().data.numpy(), it)
            ...
            self.m.logger.scalar_summary('d_probs_fake_mean', torch.sigmoid(fake_outputs[-1]).mean().cpu().data.numpy(), it)
