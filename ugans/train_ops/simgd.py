...
class SimGD(Map):
    ...
    def map(self, real_data, fake_data, data_att, atts, it=None):
        ...
        if self.m.logger is not None and it is not None:
            ...
            self.m.logger.histo_summary('d_probs_real', torch.nn.sigmoid(real_outputs[-1]).cpu().data.numpy(), it)
            ...
            self.m.logger.histo_summary('d_probs_fake', torch.nn.sigmoid(fake_outputs[-1]).cpu().data.numpy(), it)
