...
class Manager(object):
    def __init__(self, data, G, F_att, F_lat, D, D_dis, params, to_gpu, logger=None):
        ...
        if params['att_type'] == 0:
            ...
            wrel = self.to_gpu(Variable(torch.from_numpy(ycounts[:,0]/ycounts[:,1]).float()))
            ...
            self.att_loss = lambda pred, true: torch.mean(-(wrel*true*torch.log(pred+1e-10) + (1-true)*torch.log(1-pred+1e-10)))
