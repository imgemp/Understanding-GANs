...
class Manager(object):
    def __init__(self, data, G, F_att, F_lat, D, D_dis, params, to_gpu, logger=None):
        ...
        if params['att_type'] == 0:
            ...
            self.att_loss = lambda pred, true: torch.mean(10. * torch.exp(-m*true-b) * (pred-true)**2.)
        elif params['att_type'] == 2:
            m = self.to_gpu(Variable(torch.from_numpy(data.powerfits[:,0]).float()))
            b = self.to_gpu(Variable(torch.from_numpy(data.powerfits[:,1]).float()))
            self.att_loss = lambda pred, true: torch.mean(10. * torch.exp(-m*true-b) * torch.abs(pred-true))
        else:
