...
# DCGAN with Upsample+Conv instead of TransConv (uses different kernel size for generator than small2.py)
class Shapes(Data):
    def __init__(self, **kwargs):
        super(Shapes, self).__init__()
        self.shape = kwargs['sub_domain']
...
class Generator(Net):
    def __init__(self, input_dim, n_hidden=128, output_dim=64, n_layer=None, nonlin=None):
        ...
        self.main = nn.Sequential(
            ...
            nn.Conv2d(input_dim, output_dim * 8, kernel_size=2, stride=1, padding=1, bias=True),
            nn.BatchNorm2d(output_dim * 8),
            nn.ReLU(True),
            # state size. (output_dim*8) x 4 x 4
            nn.Upsample(scale_factor = 2, mode='bilinear'),
            # nn.ReflectionPad2d(1),
            nn.Conv2d(output_dim * 8, output_dim * 4, kernel_size=5, stride=1, padding=2, bias=False),
            ...
            nn.Upsample(scale_factor = 2, mode='bilinear'),
            # nn.ReflectionPad2d(1),
            nn.Conv2d(output_dim * 4, output_dim * 2, kernel_size=5, stride=1, padding=2, bias=False),
            nn.BatchNorm2d(output_dim * 2),
            nn.ReLU(True),
            # state size. (output_dim*2) x 16 x 16
            nn.Upsample(scale_factor = 2, mode='bilinear'),
            # nn.ReflectionPad2d(1),
            nn.Conv2d(output_dim * 2,     output_dim, kernel_size=5, stride=1, padding=2, bias=False),
            nn.BatchNorm2d(output_dim),
            nn.ReLU(True),
            # state size. (output_dim) x 32 x 32
            nn.Upsample(scale_factor = 2, mode='bilinear'),
            # nn.ReflectionPad2d(1),
            nn.Conv2d(    output_dim,      1, kernel_size=5, stride=1, padding=2, bias=False),
            nn.Softplus(),
            nn.Tanh()
