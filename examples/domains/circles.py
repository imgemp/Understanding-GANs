...
class Circles(Data):
    ...
    def sample_att(self, batch_size, dim=64):
        ...
        for b in range(batch_size):
            ...
            image, label = result  # label = px, py, radius
            label = np.array(label[0][1])
            label[:,:2] = (255-label[:,:2])/255.
            label[:,2] = (2*label[:,2]-min_size)/(max_size-min_size)
            samples += [np.concatenate([((255-image)/255.).astype('float32').flatten(), label]).astype('float32')]
        return torch.from_numpy(np.vstack(samples))
...
class Generator(Net):
    def __init__(self, input_dim, n_hidden=128, output_dim=64, n_layer=None, nonlin=None):
        ...
        self.convt_4 = nn.ConvTranspose2d(n_hidden, 1, kernel_size=[5,5], stride=[2,2], padding=2, output_padding=1)
...
class AttExtractor(Net):
    ...
    def __init__(self, input_dim, output_dim, image_dim=64, n_hidden=128, n_layer=None, nonlin=None, quad=False):
        ...
        self.conv_1a = nn.Conv2d(1, n_hidden, kernel_size=[5,5], stride=[2,2], padding=2)
        self.conv_1b = nn.Conv2d(n_hidden, n_hidden, kernel_size=[3,3], stride=[1,1], padding=1)
        self.conv_1c = nn.Conv2d(n_hidden, n_hidden, kernel_size=[3,3], stride=[1,1], padding=1)

        self.conv_2a = nn.Conv2d(n_hidden, n_hidden, kernel_size=[5,5], stride=[2,2], padding=2)
        ...
        self.conv_3a = nn.Conv2d(n_hidden, n_hidden, kernel_size=[5,5], stride=[2,2], padding=2)
        ...
        self.conv_4a = nn.Conv2d(n_hidden, n_hidden, kernel_size=[5,5], stride=[2,2], padding=2)
        ...
        output = nn.Linear(image_dim//16 * image_dim//16 * n_hidden, output_dim)
    ...
    def forward(self, x):
        ...
        if self.first_forward: print('AttExtractor output shapes:')
        if self.first_forward: print(x.shape)

        out = self.conv_1a(x)
        if self.first_forward: print(out.shape)
        dout = self.conv_1b(self.lrelu(out))
        if self.first_forward: print(dout.shape)
        out = out + 1e-1*self.conv_1c(self.lrelu(dout))
        if self.first_forward: print(out.shape)
        ...
        if self.first_forward: print(dout.shape)
        out = out + 1e-1*self.conv_2c(self.lrelu(dout))
        if self.first_forward: print(out.shape)
        ...
        if self.first_forward: print(dout.shape)
        out = out + 1e-1*self.conv_3c(self.lrelu(dout))
        if self.first_forward: print(out.shape)

        out = self.conv_4a(out)
        if self.first_forward: print(out.shape)
        dout = self.conv_4b(self.lrelu(out))
        if self.first_forward: print(dout.shape)
        out = out + 1e-1*self.conv_4c(self.lrelu(dout))
        if self.first_forward: print(out.shape)

        out = out.view(-1, self.image_dim//16 * self.image_dim//16 * self.n_hidden)
        out = self.output(out)
        self.first_forward = False
...
class LatExtractor(Net):
    ...
    def forward(self, x):
        x = x.view(-1, 1, self.image_dim, self.image_dim)
        if self.first_forward: print('LatExtractor output shapes:')
        if self.first_forward: print(x.shape)
